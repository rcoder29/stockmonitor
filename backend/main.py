...
from edgar_utils import (
    _session, _safe_float, _finite_or_none, _edgar_req, _edgar_filing_xml,
    _build_ticker_map, _parse_nport_xml, _get_cik, _TICKER_RE, _fetch_opp_quote,
)
from routers import (
    corporate_bonds, convertible_bonds, treasury as treasury_router,
    merger_arb, spacs,
)
# Net Market Exposure (below) reports Merger Arb / SPAC capital as separate
# event-driven sleeves by calling these route functions directly.
...
@app.post("/api/cppi", status_code=201)
...
@app.delete("/api/cppi", status_code=204)
def delete_cppi():
    with db_session() as db:
        old_ids = [r.id for r in db.query(CppiStrategy).all()]
        if old_ids:
...
@app.get("/api/portfolio/net-exposure")
...
# _edgar_req/_localname_find/_localname_iter/_build_ticker_map/
# _lookup_ticker_by_name/_parse_nport_xml/_edgar_filing_xml live in
# edgar_utils.py (imported above) — shared with SEC Filings and Corporate/
...
# _build_ticker_map / _lookup_ticker_by_name live in edgar_utils.py (imported
# above). The block below (_edgar_search_funds etc.) is Fund-Holdings-Explorer-
